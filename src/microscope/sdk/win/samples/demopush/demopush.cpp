...
public:
	...
	void OnAutoExposure(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			int mode = 0;
			if (SUCCEEDED(Toupcam_get_AutoExpoEnable(m_hcam, &mode)))
			{
				mode = !mode;
				Toupcam_put_AutoExpoEnable(m_hcam, mode);
				UISetCheck(ID_CONFIG_AUTOEXPOSURE, mode ? 1 : 0);
				UIEnable(ID_CONFIG_EXPOSURETIME, !mode);
			}
		}
	}
	...
	void OnDataCallbackV4(const void* pData, const ToupcamFrameInfoV3* pInfo, int bSnap)
	{
		if (NULL == pData)
	...
	static void __stdcall StaticOnDataCallbackV4(const void* pData, const ToupcamFrameInfoV3* pInfo, int bSnap, void* pCallbackCtx)
	{
			...
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Toupcam_Stop(m_hcam)))
				{
					...
					if (SUCCEEDED(Toupcam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						...
						Toupcam_StartPushModeV4(m_hcam, StaticOnDataCallbackV4, this, StaticOnEventCallback, this);
					}
				}
			}
		}
