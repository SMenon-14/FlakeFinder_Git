...
# ==== Extra Modbus addresses for UI actions/plots ====
...
def write_register(client, address, value, slave=10):
    """Write a holding register. Scales by 0.1 where needed."""
    # If you already have a list of addresses that need 0.1 scaling, keep it.
    # 3000 (Target SP1) expects 0.1 °C, so it should be covered by your logic
    # or you can just always send int(value*10) for temps.
    try:
...
def write_coil(client, address, value, slave=10):
    """Write a single coil (on/off). No scaling."""
    try:
...
class TemperatureControlApp:
    def _build_status_bar(self):
        bar = tk.Frame(self.master, bg=WHITE_BG, bd=1, relief="solid")
    ...
    def __init__(self, master):
        ...
        ttk.Style().configure("TEntry",  padding=(2,2))
        ...
        self.times = []
        self.temps = []
        self.set_vals = []  # track set temperature
    ...
    def _build_layout(self):
        main = tk.Frame(self.master, bg=WHITE_BG)
        main.pack(fill="both", expand=True)

        # Header
        header = tk.Frame(main, bg=WHITE_BG)
        header.pack(fill="x")
        tk.Label(header, text="Temperature Controller", font=BASE_FONT_BOLD, bg=WHITE_BG, fg="black").pack(side=tk.LEFT, padx=12, pady=8)
    ...
    def _build_config_panel(self):
        ...
        cfg.pack(pady=(4,10), fill="x")
        ttk.Separator(self.left_frame, orient="horizontal").pack(fill="x", padx=6, pady=(0,6))
        ...
        # --- Added: Start Heating button (coil write) ---
        try:
            self.btn_start = tk.Button(
                cfg,
                text=f"Start: {'ON' if getattr(self, 'start_on', False) else 'OFF'}",
                **std_btn,
                command=self.on_toggle_start
            )
            self.btn_start.grid(row=0, column=2, padx=8, pady=4)
        except Exception:
            # Fallback if this row/column isn't available
            self.btn_start = tk.Button(cfg, text="Start: OFF", **std_btn, command=self.on_toggle_start)
            self.btn_start.pack(padx=8, pady=4)


    def on_toggle_start(self):
        """Toggle Start Heating coil (10010). Tracks state locally and updates button text."""
        global client
        if not client:
    ...
    def on_toggle_start_old(self):
        """Write the Start Switch coil TRUE using current Modbus client."""
        if not client:
            print("[Init] Not connected yet.")
            return
        try:
            ...
            st = 10
        ok = write_coil(client, START_SWITCH_COIL, True, slave=st)
        if ok:
            print("[Coil] Start Heating -> TRUE")
        else:
            print("[Coil] Failed to set Start Heating")


    def _build_chart_panel(self):
        self.fig, self.ax = plt.subplots(figsize=(6.5, 4.2))
    ...
    def add_temps_to_plot(self, actual_c, set_c):
        """Push (actual,set) into the plot buffers and redraw both lines."""
        now = datetime.now()
        if (now - self.last_plot_ts).total_seconds() < 1.0:
            return
        self.last_plot_ts = now
        ...
        if set_c is not None:
            self.set_vals.append(set_c)
        else:
            self.set_vals.append(self.set_vals[-1] if self.set_vals else None)
